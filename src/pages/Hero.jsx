...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen w-full flex flex-col items-center justify-start text-center px-4 sm:px-6 py-12 sm:py-20">
      <HubBackground />
      <motion.div
      ...
      <motion.div
        ...
        className="mb-10 sm:mb-16 w-full max-w-3xl"
      >
        <div className="relative rounded-2xl p-[2px] overflow-hidden shadow-lg">
          <div className="relative z-10 bg-white/90 text-black rounded-2xl p-6 sm:p-8 shadow-lg">
            <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-center">Добро пожаловать!</h2>
      ...
      {/* Карточки */}
      <div className="mb-10 w-full max-w-6xl px-2 sm:px-4 flex flex-col sm:flex-row justify-center gap-4 sm:gap-6">
        {features.map((card, i) => (
          <motion.div
            key={i}
            ...
            "
          >
            <h3 className="neon-blue text-base sm:text-lg font-bold text-blue-900 mb-2">{card.title}</h3>
            <p className="neon-blue text-sm sm:text-base text-black">{card.desc}</p>
          </motion.div>
        ))}
      </div>
      ...
      {/* Текст и кнопка */}
      <motion.div>
        <p className="mt-6 text-base sm:text-lg max-w-2xl mx-auto text-white text-center px-2">
